...
Table above = **bare-local default**. Conductor/exe.dev shift ports per worktree (Conductor sets `CONDUCTOR_PORT`; exe.dev base 3000). Real per-worktree URLs/ports live in `.dev-env.md` at repo root — gitignored, regenerated by `pnpm db:env:development` (every setup script runs it).
