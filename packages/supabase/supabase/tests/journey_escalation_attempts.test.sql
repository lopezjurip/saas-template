...
--      clear `membership_revoked_at` because the write policy is DB-backed
...
-- ============================================================
-- Seeds: capture relevant membership ids and create an unaffiliated user Eve.
-- ============================================================
...
--
-- Step 1: Alice (admin) revokes Bob's membership.
...
update public.memberships
  ...
  where membership_id = (select bob_org1 from _mids);

-- Quick check Alice could see the revoked row reflected.
select isnt(
  (select membership_revoked_at from public.memberships
     where membership_id = (select bob_org1 from _mids)),
  null,
  'admin successfully stamped revoked_at on the target membership'
);
...
update public.memberships
  set membership_revoked_at = null
  where membership_id = (select bob_org1 from _mids);
...
prepare bob_insert_harmless_preset as
  insert into public.permission_presets (
    ...
    1, 'Bob Harmless Preset', array['vacations_request']::extensions.citext[]
  );
select throws_ok(
...
prepare bob_corrupt_global as
  update public.permission_presets
    set permission_preset_slugs = array['*']::extensions.citext[]
...
select isnt(
  (select permission_preset_slugs from public.permission_presets
     where permission_preset_name = 'Empleado' and organization_id is null),
  array['*']::extensions.citext[],
  'global Empleado preset unchanged after Bob attempted UPDATE (USING filters him out)'
);
