-- Business invariants on organization_memberships / permission_grants:
...
--   Org 1 (acme):   Alice = '*' (sole admin), Bob = 'presets_manage' (not admin).
--   Org 2 (globex): Alice = 'presets_manage' (no admin perm).
...
select throws_ok(
  ...
  'P0001',
  null,
  'last admin cannot revoke wildcard permission via permission_grants delete'
);
...
-- Non-admin permission deletion is allowed (Alice removing presets_manage from her org 2 grants).
select lives_ok(
  format(
    $$ delete from public.permission_grants
...
-- Alice (sole admin via '*') grants herself members_manage.
select lives_ok(
  format(
...
-- Alice inserts Bob's '*' into permission_grants. She currently holds members_manage → write policy passes.
set local role authenticated;
set local request.jwt.claims to '{
  "sub": "00000000-0000-0000-0000-00000000a11c",
  "app_metadata": {
