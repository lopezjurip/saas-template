-- pgTAP tests for authz.grants table structure and CHECK constraints.
...
-- two subjects set → rejected by authz_grants_one_subject CHECK
select throws_ok(
...
-- two objects set → rejected by authz_grants_one_object CHECK
select throws_ok(
...
-- profile subject with NO object → rejected by authz_grants_all_orgs_only_agency CHECK
select throws_ok(
