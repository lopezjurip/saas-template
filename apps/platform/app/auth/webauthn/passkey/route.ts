...
export async function POST(request: NextRequest) {
  ...
  const { data: profileId, error: rpcError } = await supabase.rpc("profile_id_by_email", { email_to_check: email });
  if (rpcError) {
  ...
  const credentials = await listWebAuthnCredentialsForProfile(supabase, profileId);
  if (credentials.length === 0) {
    log.info("sign-in challenge for user with no passkeys", { email, profile_id: profileId });
    return NextResponse.json({ message: "Usuario sin passkey" }, { status: 404 });
  }
  ...
  if (!challenge?.webauthn_challenge_id) {
    log.error("anonymous challenge insert returned no row", { email, profile_id: profileId });
  ...
  log.info("sign-in challenge issued", { email, profile_id: profileId, credential_count: credentials.length });
