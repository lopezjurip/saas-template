...
async function continueWithPhone(value: string, next: string): Promise<never> {
  ...
  if (!AUTH_EXPOSE_ACCOUNT_EXISTENCE) {
    ...
    redirect(`/auth/phone?${qs.toString()}`);
  }
...
async function continueWithDocument(value: string, next: string): Promise<never> {
  ...
  if (!doc) redirect(`/auth/document?error=invalid_document&next=${encodeURIComponent(next)}`);
  redirect(`/auth/document?value=${encodeURIComponent(doc)}&next=${encodeURIComponent(next)}`);
}
