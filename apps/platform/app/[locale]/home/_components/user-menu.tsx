...
export function UserMenu({ locale, name, email }: { locale: string; name: string; email: string }) {
  ...
  return (
    <div className="fixed bottom-[18px] left-[18px] z-[5] flex flex-col gap-1">
      {open && (
        <div
          role="menu"
          className="absolute bottom-[calc(100%+8px)] left-0 flex w-[220px] flex-col gap-px rounded-md border bg-popover p-1 text-popover-foreground shadow-[0_8px_24px_rgba(0,0,0,0.12)]"
        >
          <div className="flex items-center gap-2.5 px-2.5 pt-2 pb-1.5">
            <span className="inline-flex size-8 items-center justify-center rounded-full bg-primary text-[13px] font-semibold tracking-[-0.01em] text-primary-foreground">
              {INITIALS_OF(name || email)}
            </span>
            <span className="flex min-w-0 flex-col leading-[1.2]">
              <strong className="text-[13px] font-medium text-foreground">{name || "Sin nombre"}</strong>
          ...
          <div className="flex flex-col border-b py-1">
            <Link
              ...
              className="flex h-8 items-center gap-2.5 rounded-md px-2.5 text-[13px] text-foreground hover:bg-accent"
            >
              Mi perfil
            </Link>
            <Link
              href={`/${locale}/home/account/security`}
              className="flex h-8 items-center gap-2.5 rounded-md px-2.5 text-[13px] text-foreground hover:bg-accent"
            >
              Inicio de sesión
            </Link>
          ...
          <div className="flex flex-col py-1">
            <Link
              ...
              className="flex h-8 items-center gap-2.5 rounded-md px-2.5 text-[13px] text-destructive hover:bg-accent"
            >
              Cerrar sesión
            </Link>
          </div>
      ...
      >
        <span className="inline-flex size-7 items-center justify-center rounded-full bg-primary text-[11.5px] font-semibold tracking-[-0.01em] text-primary-foreground">
          {INITIALS_OF(name || email)}
        </span>
