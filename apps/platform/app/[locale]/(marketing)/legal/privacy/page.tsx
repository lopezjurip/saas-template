...
export async function generateMetadata(props: PageProps<"/[locale]/legal/privacy">): Promise<Metadata> {
  const { locale } = await props.params;
  const base = `https://${APP_HOST}`;
  const safeLocale = IS_SUPPORTED_LOCALE(locale) ? locale : DEFAULT_LOCALE;
  const title = LEGAL_DOCS[LEGAL_LOCALE(safeLocale)].privacy.title;
  return {
    title,
    alternates: {
...
export default async function LegalPrivacyPage(props: PageProps<"/[locale]/legal/privacy">) {
  const { locale } = await props.params;
  return <LegalArticle locale={LEGAL_LOCALE(locale)} section="privacy" />;
}
