 ...
 * back to `/${locale}/home`. We deliberately do this in one place so every route stays
 * locked down and the fallback target can be changed in one edit.
 */
export function RESOLVE_AUTH_NEXT(raw: string | null, origin: string, locale: string): string {
