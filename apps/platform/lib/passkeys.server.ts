...
export async function listWebAuthnCredentialsForProfile(client: Client, profileId: string) {
  const { data, error } = await client.from("webauthn_credentials").select("*").eq("profile_id", profileId);
...
export async function getWebAuthnCredentialByExternalId(
  ...
  const { data } = await client
    ...
    .select("*")
    .eq("webauthn_credential_external_id", externalId)
    .maybeSingle();
  return data;
}
...
export async function insertWebAuthnChallenge(client: Client, params: { profile_id?: string; value: string }) {
  if (params.profile_id) {
    const { data, error } = await client
      ...
      .upsert([{ profile_id: params.profile_id, webauthn_challenge_value: params.value }], { onConflict: "profile_id" })
      .select("*")
      .maybeSingle();
    if (error) throw new Error(error.message);
    return data;
...
export async function getWebAuthnChallengeByProfile(client: Client, profileId: string) {
  const { data } = await client.from("webauthn_challenges").select("*").eq("profile_id", profileId).maybeSingle();
  return data;
}

export async function getWebAuthnChallengeById(client: Client, challengeId: string) {
  const { data } = await client
    .from("webauthn_challenges")
    .select("*")
    .eq("webauthn_challenge_id", challengeId)
    .maybeSingle();
  return data;
}

export async function deleteWebAuthnChallenge(client: Client, challengeId: string) {
  await client.from("webauthn_challenges").delete().eq("webauthn_challenge_id", challengeId);
}
...
export async function getRegistrationOptions(client: Client, user: User) {
  ...
  return generateRegistrationOptions({
    ...
    excludeCredentials: credentials.map((c) => ({
      ...
      type: c.webauthn_credential_type,
